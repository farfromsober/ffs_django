...
class ProductPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        permiso para realizar distintas acciones (GET, POST, PUT, OPTIONS, DELETE)
        """

        # el superusuario siempre puede
        # el cliente web pide primero con el método options lo que puede hacer, damos permiso siempre
        if request.user.is_superuser or view.action == 'metadata':
            return True

        # si es un 'list'/'retrieve'/'create', solo pueden usuarios autenticados.
